...
| **TC-27** | Multi-Voice Persona Selection & Prompt Tuning | 1. Click Voice Persona dropdown in Workspace toolbar.<br>2. Audition samples for 'Calm Mentor', 'Analytical Observer', 'Empathetic Friend', etc.<br>3. Select a persona.<br>4. Send a reflection. | Persona updates in ThemeContext and persists; Gemini backend route dynamically adopts persona prompt tone; assistant messages display persona badge and speak using persona pitch/rate. |
| **TC-28** | Pomodoro Reflection Timer & Chime | 1. Click "Reflection Timer" in Workspace toolbar.<br>2. Select a preset (e.g. 5m, 10m, 25m) or custom duration.<br>3. Click Play.<br>4. Let timer reach 0:00 or click +1m. | Circular progress bar counts down; synthesized Web Audio gentle chime sounds on completion; awards 'Deep Focus' milestone sticker with one-tap addition to entry. |
| **TC-29** | Journal Stickers & Milestone Badges | 1. Click "+ Sticker" in Workspace toolbar.<br>2. Search or filter by categories (Mindfulness, Insight, Emotion, Focus, Milestone).<br>3. Select stickers (e.g. Zen Flow, Eureka Insight, Breakthrough). | Stickers render as interactive colorful chips in the entry header; stickers persist in Firestore; stickers can be removed or toggled at any time. |
| **TC-30** | Sovereign Export Formats & DLP Scrubbing | 1. Click "Export Vault" or open Zero-Trust Security Modal.<br>2. Select options (DLP scrubbing, include locations, include stickers).<br>3. Choose format: Encrypted Vault (.enc.json), Sovereign JSON (.json), or Markdown Diary (.md). | Previews export statistics and SHA-256 integrity checksum; exports clean sovereign data; DLP scrubber strips PII before export when enabled. |
